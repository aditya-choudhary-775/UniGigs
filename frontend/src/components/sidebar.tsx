        ...
        {sideBarItems.map((item, index) => {
          return (
            <Link
              href={item.href}
              key={item.href}
              className={`flex items-center gap-2 rounded-md border border-black py-2 pl-2 pr-6 text-neutral-600 transition-all duration-200 hover:text-neutral-700 ${pathname.startsWith(item.href) ? "[box-shadow:-2px_2px_0px_theme(colors.neutral.500)] bg-sky-200" : "[box-shadow:-5px_5px_0px_black] bg-cyan-100 hover:bg-cyan-200"}`}
            >
              {item.icon}
